#### 🙇‍♂️ 최종본 코드에서 pipeline output이 제대로 안나오는 문제가 발생하여 nbviewer을 통해 해결했습니다. 코드를 보시려면 아래 url을 클릭해주세요.
...
1) 내부 데이터
   ...
   => 위의 2개의 데이터들은 결측치가 너무 많아 column을 만들기 힘들었다.
   
2) 외부 데이터
   - 초중고 데이터 : 동 별 고등학교 수
   - 대출 금리 데이터 : 금리 변화율
   - 서울/ 부산 평균 소득 : 동 별 평균 소득 금액
   
3) 데이터 병합 후
   - 각 column들을 조합하여 수치형 파생 변수들 생성

##### 🔄 Encoding
- 고유값이 적은 column : One-hot Encoding
